...
class Regex:
  ...
  def getLineInfo(string, matchInfo, linebreak="\n"):
    ...
    for i in range(len(matchLines)):
      lineInfo.append(MatchLineInfo(line=lineOffset + i, start=columnOffset, end=columnOffset + len(matchLines[i]), length=len(matchLines[i]), string=matchLines[i], invert=matchInfo.invert))
      columnOffset = 0
    return lineInfo
